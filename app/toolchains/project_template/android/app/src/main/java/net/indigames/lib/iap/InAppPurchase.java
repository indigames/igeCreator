...
/**
 * The InAppPurchase implements all billing functionality for our test application. Purchases
 * can happen while in the app or at any time while out of the app, so the InAppPurchase has to
 * account for that.
 * <p>
 * Since every SKU can have an individual state, all SKUs have an associated LiveData to allow their
 * state to be observed.
 * <p>
 * This InAppPurchase knows nothing about the application; all necessary information is either
 ...
 * only thing that clients of the InAppPurchase need to know are the SKUs used by their
 * application.
 * <p>
 * The BillingClient needs access to the Application context in order to bind the remote billing
 * service.
 * <p>
 * The InAppPurchase can also act as a LifecycleObserver for an Activity; this allows it to
 * refresh purchases during onResume.
 */
public class InAppPurchase implements LifecycleObserver, PurchasesUpdatedListener, BillingClientStateListener, SkuDetailsResponseListener {
    ...
    /**
     * YOUR APPLICATION PUBLIC KEY: from Google Play Console -> Monetization Setup -> Licensing.
     */
    private String appPublicKey = "";
    private static final Handler handler = new Handler(Looper.getMainLooper());
    private static volatile InAppPurchase sInstance;
    ...
    final private SingleMediatorLiveEvent<List<String>> newPurchase = new SingleMediatorLiveEvent<List<String>>();
    final private SingleMediatorLiveEvent<List<String>> purchaseConsumed = new SingleMediatorLiveEvent<List<String>>();
    ...
    /**
     * Class IAPProduct
     * Used to serialize SkuDetails to C++ equivalent object
     */
    public class IAPProduct {
        ...
        public IAPProduct(final String id, final String title, final String des, final String loc, float price, float originPrice) {
            this.id = id;
            this.title = title;
     ...
     * @param knownSKUs ProductIds list
     */
    private InAppPurchase(final String publicKey, final String[] knownSKUs) {
        knownInappSKUs = (knownSKUs == null) ? new ArrayList<>() : Arrays.asList(knownSKUs);
        knownSubscriptionSKUs = new ArrayList<>();
        knownAutoConsumeSKUs = new HashSet<>();
        appPublicKey = publicKey;
        billingClient = BillingClient.newBuilder(((Activity)SDLActivity.getContext()).getApplicationContext()).setListener(this).enablePendingPurchases().build();
     ...
     */
    public static InAppPurchase createInstance(final String publicKey, final String[] knownSKUs) {
        if (sInstance == null) {
            synchronized (InAppPurchase.class) {
                if (sInstance == null) {
                    sInstance = new InAppPurchase(publicKey, knownSKUs);
                }
            }
        }
        return sInstance;
    ...
    public static InAppPurchase getInstance() {
        if (sInstance == null) {
            Log.w(TAG, "sInstance is null, ensure to call `createInstance()` first!");
        }
        return sInstance;
    }
    ...
    private void retryBillingServiceConnectionWithExponentialBackoff() {
        handler.postDelayed(() ->
                        billingClient.startConnection(InAppPurchase.this),
    ...
    private void initializeLiveData() {
        ...
        billingFlowInProcess.postValue(false); // [IGE]: use postValue to ensure it run on main thread
    }
        ...
        switch (responseCode) {
            case BillingClient.BillingResponseCode.OK:
                ...
                if (skuDetailsList == null || skuDetailsList.isEmpty()) {
                    ...
                    forwardEvent(IAPEvent.List, IAPStatus.Succeed, null);
                } else {
                    ...
                    forwardEvent(IAPEvent.List, IAPStatus.Succeed, skuDetailsList);
                }
                break;
            ...
            case BillingClient.BillingResponseCode.ERROR:
                ...
                forwardEvent(IAPEvent.List, IAPStatus.Failed, null);
                break;
        ...
        if (null != purchases) {
            ...
            for(String sku : updatedSkus) {
                ...
                if(skuDetailsLiveData != null) {
                    skuDetailsList.add(skuDetailsLiveData.getValue());
                }
            }
            forwardEvent(IAPEvent.Purchase, IAPStatus.Succeed, skuDetailsList);
        } else{
            Log.d(TAG, "Empty purchase list.");
            forwardEvent(IAPEvent.Purchase, IAPStatus.Failed, null);
        }
     ...
     */
    public void launchBillingFlow(@NonNull String sku, String... upgradeSkus) {
            ...
            if (null != upgradeSkus && upgradeSkus.length > 0) {
                ...
                try {
                    ...
                    if (br.getResponseCode() == BillingClient.BillingResponseCode.OK) {
                        ...
                        Log.e(TAG, "Billing failed: + " + br.getDebugMessage());
                        forwardEvent(IAPEvent.Purchase, IAPStatus.Failed, new ArrayList<SkuDetails>());
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                    forwardEvent(IAPEvent.Purchase, IAPStatus.Failed, new ArrayList<SkuDetails>());
    ...
    public void onPurchasesUpdated(@NonNull BillingResult billingResult,
        ...
        switch (billingResult.getResponseCode()) {
            ...
            case BillingClient.BillingResponseCode.ITEM_ALREADY_OWNED:
                ...
                forwardEvent(IAPEvent.Purchase, IAPStatus.AlreadyPurchased, null);
                break;
            case BillingClient.BillingResponseCode.DEVELOPER_ERROR:
    ...
    private boolean isSignatureValid(@NonNull Purchase purchase) {
        return Security.verifyPurchase(purchase.getOriginalJson(), purchase.getSignature(), appPublicKey);
    }
    ...
    private enum IAPStatus {
        Failed(-1),
        Succeed(0),
        NotInitialized(1),
        IdentifierNotFound(2),
        InvalidReceipt(3),
        NoRestorablePurchase(4),
        AlreadyPurchased(5);

        IAPStatus(final int val) {
            value = val;
        }
        public int getValue() { return value; }
        private final int value;
    ...
    /**
     * Forward event to C++ using JNI Bridge
     */
    private void forwardEvent(final IAPEvent event, final IAPStatus status, final List<SkuDetails> skus)
    {
        List<IAPProduct> products = new ArrayList<IAPProduct>();
        if(skus != null) {
            for (SkuDetails sku : skus) {
                long price = sku.getPriceAmountMicros() / 1000000;
                long originPrice = sku.getOriginalPriceAmountMicros() / 1000000;
                IAPProduct product = new IAPProduct(sku.getSku(), sku.getTitle(), sku.getDescription(), sku.getPriceCurrencyCode(), (float)price, (float)originPrice);
                products.add(product);
            }
        }
        nativeForwardEvent(event.getValue(), status.getValue(), (new Gson()).toJson(products));
    }
    
    public native void nativeForwardEvent(final int event, final int status, final String params);
