...
namespace ige::creator
{
    ...
    void Button::_drawImpl()
    {
        ...
        else if (m_color.x >= 0.f && ImGui::ColorButton((m_label + getIdAsString()).c_str(), m_color, 0, m_size))
        {
            getOnClickEvent().invoke(this);
        }
        else if (m_label != "" && ImGui::ButtonEx((m_label + getIdAsString()).c_str(), m_size, !m_bEnabled ? ImGuiItemFlags_Disabled : 0))
        {
            getOnClickEvent().invoke(this);
        }
    }
