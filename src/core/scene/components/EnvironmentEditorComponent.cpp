...
void EnvironmentEditorComponent::drawEnvironmentComponent() {
    ...
    auto shadowColor = Vec4(comp->getProperty<Vec3>("shadowCol", {NAN, NAN, NAN}), 1.f);
    ...
    auto size = comp->getProperty<Vec2>("shadowSize", { NAN, NAN });
    ...
    std::array density = { comp->getProperty<float>("shadowDensity", NAN) };
    ...
    std::array wideness = { comp->getProperty<float>("shadowWideness", NAN) };
    ...
    std::array bias = { comp->getProperty<float>("shadowBias", NAN) };
    ...
    auto fogColor = Vec4(comp->getProperty<Vec3>("fogCol", {NAN, NAN, NAN}), comp->getProperty<float>("fogAlpha", 1.f));
    ...
    std::array fogNear = { comp->getProperty<float>("fogNear", NAN) };
    ...
    std::array fogFar = { comp->getProperty<float>("fogFar", NAN) };
