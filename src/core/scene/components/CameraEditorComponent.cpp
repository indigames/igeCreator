...
void CameraEditorComponent::drawCameraComponent() {
    ...
    if (comp->getProperty<bool>("ortho", false))
    {
        ...
        std::array orthorW = { comp->getProperty<float>("orthoW", 1.f) };
        ...
        std::array orthorH = { comp->getProperty<float>("orthoH", 1.f) };
    ...
    // FOV - Near - Far
    std::array fov = { comp->getProperty<float>("fov", 1.f) };
    ...
    std::array camNear = { comp->getProperty<float>("near", 0.1f) };
    ...
    std::array camFar = { comp->getProperty<float>("far", 0.1f) };
    ...
    {
        auto targetPos = comp->getProperty<Vec3>("target", { 0.f, 0.f, 0.f });
    ...
    {
        // Pan - Tilt - Roll
        std::array pan = { RADIANS_TO_DEGREES(comp->getProperty<float>("pan", 0.f)) };
        ...
        std::array tilt = { RADIANS_TO_DEGREES(comp->getProperty<float>("tilt", 0.f)) };
        ...
        std::array roll = { RADIANS_TO_DEGREES(comp->getProperty<float>("roll", 0.f)) };
    ...
    // Aspect Ratio
    std::array ratio = { comp->getProperty<float>("aspect", 0.f) };
    ...
    // Up vector
    std::array upArr = { comp->getProperty<int>("up", 0) };
    m_cameraCompGroup->createWidget<Drag<int>>("Up", ImGuiDataType_Float, upArr, 1, 0)->getOnDataChangedEvent().addListener([this](auto val) {
        getComponent<CompoundComponent>()->setProperty("up", val[0]);
    });

    // Screen scale
    auto sScale = comp->getProperty<Vec2>("scrScale", {1.f, 1.f});
    ...
    // Screen offset
    auto sOff = comp->getProperty<Vec2>("scrOff", { 0.f, 0.f });
    ...
    // Screen radian
    auto sRad = comp->getProperty<float>("scrRad", 0.f);
