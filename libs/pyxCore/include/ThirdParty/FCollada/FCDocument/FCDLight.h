...
public:
	...
	enum LightType
	{
		...
		FOG				/**< フォグの居場所がないので、とりあえずライト扱いでここに入れておく*/
	};
...
private:
	...
	DeclareParameterAnimatable(FMVector3, FUParameterQualifiers::COLOR, shadowColor, FC("ShadowColor")); // Non-standard COLLADA
	...
	// Fog-specific light parameters
	DeclareParameterAnimatable(float, FUParameterQualifiers::SIMPLE, fogFar, FC("Fog Far Distance"));
	DeclareParameterAnimatable(float, FUParameterQualifiers::SIMPLE, fogNear, FC("Fog Near Distance"));
	DeclareParameterAnimatable(float, FUParameterQualifiers::SIMPLE, fogAlpha, FC("Fog Alpha Value"));
...
public:
	...
	*/
	...
	/**
		フォグ用パラーメータ
	*/
	FCDParameterAnimatableFloat& GetFogFar() { return fogFar; }
	const FCDParameterAnimatableFloat& GetFogFar() const { return fogFar; }
	void SetFogFar(float dist) { fogFar = dist; SetDirtyFlag(); }

	FCDParameterAnimatableFloat& GetFogNear() { return fogNear; }
	const FCDParameterAnimatableFloat& GetFogNear() const { return fogNear; }
	void SetFogNear(float dist) { fogNear = dist; SetDirtyFlag(); }

	FCDParameterAnimatableFloat& GetFogAlpha() { return fogAlpha; }
	const FCDParameterAnimatableFloat& GetFogAlpha() const { return fogAlpha; }
	void SetFogAlpha(float alpha) { fogAlpha = alpha; SetDirtyFlag(); }

	FCDParameterAnimatableColor3& GetShadowColor() { return shadowColor; }
	const FCDParameterAnimatableColor3& GetShadowColor() const { return shadowColor; }
	void SetShadowColor(const FMVector3& col) { shadowColor = col; SetDirtyFlag(); }
	void SetShadowColor(float r, float g, float b) { shadowColor = FMVector3(r, g, b); SetDirtyFlag(); }


	float GetLightRange() const { return range; }
	void SetLightRange(float r) { range = r;}
