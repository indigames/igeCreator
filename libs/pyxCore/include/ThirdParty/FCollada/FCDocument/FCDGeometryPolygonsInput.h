...
private:
	...
	DeclareParameter(uint32, FUParameterQualifiers::SIMPLE, channel, FC("Stream Channel"));
...
public:
	...
	/** Retrieves the channel within the interlaced COLLADA indices for this input's indices.
		Changing this value after the input's construction is not recommended.
		@param _offset The offset within the interlaced COLLADA indices for this input's indices. */
	inline void SetChannel(uint32 _channel) { channel = _channel; }

	/** Retrieves the channel within the interlaced COLLADA indices for this input's indices.
		@return The interlaced indices offset. */
	inline uint32 GetChannel() const { return channel; }
