...
namespace FUDaeWriter
{
	...
	/** Writes out a COLLADA input element.
		...
		@param target The optional target attribute value.
			This is a shader vertex input channel value/ 
		@return The created \<input\> XML tree node. */
	xmlNode* AddInput(xmlNode* parent, const char* sourceId, const char* semantic, int32 offset=-1, int32 set=-1, int32 target=-1);
	inline xmlNode* AddInput(xmlNode* parent, const fm::string& sourceId, const char* semantic, int32 offset=-1, int32 set=-1, int32 target=-1) { return AddInput(parent, sourceId.c_str(), semantic, offset, set,target); } /**< See above. */
