...
namespace pyxie
{
	...
	private:
		...
		void GetVertexesAndPolygons(const FCDGeometryPolygons *polys, std::map<UniqueVert, uint32_t, UniqueVertCompare> *outUniqueVerts, std::vector<int32_t> *outPolyList);
		...
		bool GetMaterialParam(const FCDEffectParameter* fparam, FigureMaterialParam& oparam, pyxieEditableFigure* editableFigure);
	};
