...
#define	PYXIE_SAFE_FREE(p) { if(p){pyxie::PYXIE_FREE(p); (p) = 0;} }
	...
	void PYXIE_EXPORT StartTextInput(const char* text);
	void PYXIE_EXPORT StopTextInput();
	void PYXIE_EXPORT SetInputText(const char* text);
	const char PYXIE_EXPORT* GetInputText(int* size);
    void PYXIE_EXPORT InputTextBackSpace();
    bool PYXIE_EXPORT IsVirtualKeyboardShown();
}
