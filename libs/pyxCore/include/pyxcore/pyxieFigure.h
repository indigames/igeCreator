...
namespace pyxie
{
	...
	public:
		...
		FigureMaterial* GetMaterial(uint32_t index) {
		...
		void KillJointTransform(int index);


		FigureMesh* GetMesh(int index);
		...
		void ResetMeshBuffer(uint32_t index, bool delVertex, bool delIndex, bool delAttribute);
...
*/
	...
	private:
		...
		void DeleteVertexBuffer(MeshInfo& info, bool delVertex, bool delIndex, bool delAttribute);
		void GenVBO(FigureMesh & mesh, MeshInfo & info);
		int GenVAO(pyxieShader* shader, FigureMesh& mesh, uint32_t verticesUid);


		void DrawMesh(FigureMesh& mesh, MeshInfo& info, MeshParam& param, uint32_t pass);
		bool LoopMesh(int& i);
	};
}