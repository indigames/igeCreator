...
namespace pyxie {
	...
	public:
		...
		enum ReferenceMapChannel {
			...
			OverlayDiffuseMapAlphaChannel,	//
			OverlayDiffuseMapRedChannel,	//
			OverlayNormalMapAlphaChannel,	//
			OverlayNormalMapRedChannel		//
		};
		...
		struct _ShaderDesc
		{
			...
			union {
				struct {
					...
					uint32_t UVSCROLL_SET0 : 1;						//_, 1
					uint32_t UVSCROLL_SET1 : 1;						//_, 2
					uint32_t UVSCROLL_SET2 : 1;						//_, 3
					uint32_t UVOFFSET_SET0 : 1;						//_, 4
					uint32_t OL_PARALLAX_MAP_ENABLE : 1;			//_, P
					uint32_t OL_HEIGHT_MAP_CHANNEL : 4;				//_, n,d
					uint32_t UVOFFSET_SET1 : 1;						//_, 5
					uint32_t UVOFFSET_SET2 : 1;						//_, 6
					...
					uint32_t UVROTATION_SET0 : 1;					//_,1
					uint32_t UVROTATION_SET1 : 1;					//_,2
					uint32_t UVROTATION_SET2 : 1;					//_,3
					uint32_t UVSCALE_SET0 : 1;						//_,1
					uint32_t UVSCALE_SET1 : 1;						//_,2
					uint32_t UVSCALE_SET2 : 1;						//_,3
		...
		struct _ShaderInterpolationInfo
		{
			...
			union {
				struct {
					...
					uint32_t LIGHT_VIEW_POSITION : 1;
					uint32_t LIGHT_VIEW_TRANSFORM : 1;
					uint32_t USE_LIGHT_VIEW_MAPCODE : 1;
					uint32_t DIFFUSE_COLOR_ENABLE : 1;
					uint32_t SHADOW_COLOR_ENABLE : 1;
				};
				uint32_t buffer[8];
			};
		};
	...
	public:
		pyxieShaderDescriptor() {
			...
			desc.Z_REFERENCE_ENABLE = 1;
			desc.Z_WRITE_ENABLE = 1;
		}
		...
		void SetColorTextureUVSet(int rgb, int a);
		void SetNormalTextureUVSet(int rgb, int a);
		void SetLightTextureUVSet(int rgb, int a);
		void SetOverlayColorTextureUVSet(int rgb, int a);
		void SetOverlayNormalTextureUVSet(int rgb, int a);

		void SetUVOffset(int channel, bool enable);
		void SetProjectionMapping(int channel, bool enable);
		void SetUnifiedColor(bool enable);
		void SetShadow(bool make, bool receive, bool depthShadow);
		void SetShadowMakeShader(bool make, bool depthShadow);
		void SetUVScroll(int channel, bool enable);
		void SetDistortion(bool enable);

		void SetAlphaTest(bool enable);
		void SetDoubleside(bool enable);
		void SetFrontFaceCulling(bool enable);
		void SetZTest(bool enable);
		void SetZWritee(bool enable);
		...
		inline bool IsReceiveShadow() { return desc.RECEIVE_SHADOW_ENABLE; }
		inline bool IsDepthShadow() { return desc.DEPTH_SHADOW_ENABLE; }

		inline bool IsAlphaTestEnable() { return desc.ALPHA_TEST_ENABLE; }
		inline bool IsDoubleside() { return desc.DOUBLE_SIDE; }
		inline bool IsFrontFaceCulling() { return desc.FRONT_FACE_CULLING; }
		inline bool IsZTestEnable() { return desc.Z_REFERENCE_ENABLE; }
		inline bool IsZWriteeEnable() { return desc.Z_WRITE_ENABLE; }

		inline bool IsAlphaBlendEnable() { return desc.ALPHA_BLENDING_ENABLE; }
		inline int GetAlphaBlendOP() { return desc.ALPHA_BLEND_OP; }
		...
		inline int GetLightTextureRGBUVSet() { return desc.lightMapUVSet / 3; }
		inline int GetOverlayColorTextureRGBUVSet() { return desc.overlayColorMapUVSet / 3; }
		inline int GetOverlayNormalTextureRGBUVSet() { return desc.overlayNormalMapUVSet / 3; }
		inline int GetColorTextureAlphaUVSet() { return desc.colorMapUVSet % 3; }
		inline int GetNormalTextureAlphaUVSet() { return desc.normalMapUVSet % 3; }
		inline int GetLightTextureAlphaUVSet() { return desc.lightMapUVSet % 3; }
		inline int GetOverlayColorTextureAlphaUVSet() { return desc.overlayColorMapUVSet % 3; }
		...
		static inline int DescSize() { return sizeof(_ShaderDesc); }
