...
class PYXIE_EXPORT pyxieParticle : public pyxieFigure
{
	...
	public:
		pyxieParticle(b2Vec2* posBuff = NULL, b2Color* colorBuff = NULL, float* weightBuff = NULL, int count = 0, float size = -1, float scale = 1.0);
		~pyxieParticle();

		void Init(float scale = 1.0);
		void UpdateParticles(b2Vec2* posBuff = NULL, b2Color* colorBuff = NULL, float* weightBuff = NULL, int count = 0, float size = -1);
		virtual void Render();
		void RenderToScreen();
		void Blur();
		...
		inline static pyxieParticle *new_particle(b2Vec2 *posBuff = NULL, b2Color *colorBuff = NULL, float* weightBuff = NULL, int count = 0, float size = -1, float scale = 1.0)
		{
			return new pyxieParticle(posBuff, colorBuff, weightBuff, count, size, scale);
		}
